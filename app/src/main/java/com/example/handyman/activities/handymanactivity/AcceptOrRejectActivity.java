            ...
            case R.id.btnAccept:
                ...
                if (notApproved.equals("not approved")) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            new AlertDialog.Builder(AcceptOrRejectActivity.this)
...
//node for approved leave
        ...
        requestDbref.updateChildren(approve).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {

                    assert Id != null;
                    Approved.child(Id).setValue(approvedLeave).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
...
//leave accepted node
        ...
        final String Id = requestDbref.push().getKey();
        //random key for leave accepted
        requestDbref.updateChildren(rejectxx).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    assert Id != null;
                    //create and update the node
                    Rejected.child(Id).setValue(reject).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
